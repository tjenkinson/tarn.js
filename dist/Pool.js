...
class Pool {
  constructor(opt) {
    ...
    if (opt.idleTimeoutMillis !== 0 && !utils_1.checkOptionalTime(opt.idleTimeoutMillis)) {
      throw new Error(
      ...
      .reflect(
        Promise.all(this.pendingCreates.map(create => utils_1.reflect(create.promise)))
          ...
          .then(() => {
            ...
            return Promise.all(
              this.free.map(free => utils_1.reflect(this._destroy(free.resource)))
            );
          })
          .then(() => {
  ...
  _doAcquire() {
    ...
    while (this._canAcquire()) {
      ...
      const abortAbleValidation = new PendingOperation_1.PendingOperation(
